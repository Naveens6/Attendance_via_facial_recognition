...
def train_model():
    ...
    model = keras.Sequential([
        ...
        layers.Dense(len(set(labels)), activation='softmax')  # Output layer with number of classes
    ])

    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

    model.fit(faces, labels, epochs=10)  # You may need to adjust the number of epochs
...
def identify_face(facearray):
    ...
    return model.predict_classes(facearray.reshape(1, 50, 50, 3))[0]
